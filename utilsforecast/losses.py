...
# %% auto 0
__all__ = ['mae', 'mse', 'rmse', 'bias', 'mape', 'smape', 'mase', 'rmae', 'msse', 'rmsse', 'quantile_loss', 'mqloss', 'coverage', 'calibration',
           'scaled_crps']
...
def rmsse(
    ...
    """Root Mean Squared Scaled Error (RMSE)

    RMSSE is the square root of the MSSE"""
    res = msse(df, models, id_col, target_col)
    if isinstance(res, pd.DataFrame):
        res[models] = res[models].pow(0.5)
    else:
        res = res.with_columns(*[pl.col(c).pow(0.5) for c in models])
