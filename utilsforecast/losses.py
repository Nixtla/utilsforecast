...
def wape(
    ...
    the total absolute error by the total absolute value of actuals. This makes
    it robust to zero values and gives more weight to higher-volume items.

    References
    ----------
    - Kolassa & Schütz (2007). "Advantages of the MAD/Mean ratio over the MAPE."
    - https://robjhyndman.com/hyndsight/wmape/
    """

    # Step 1: per-group sum of absolute errors
    def gen_numer(model):
    ...
    # Step 2: per-group sum of |y| (denominator)
    group_cols = _get_group_cols(df=df, id_col=id_col, cutoff_col=cutoff_col)
    denom = (
        nw.from_native(df)
        .select([*group_cols, nw.col(target_col).abs().alias("__abs_y__")])
        .group_by(*group_cols)
        .agg(nw.col("__abs_y__").sum())
        .sort(*group_cols)
    )

    # Step 3: join and divide
    result = nw.from_native(numer).join(denom, on=group_cols, how="left")
    for model in models:
        result = result.with_columns(
            (nw.col(model) / _zero_to_nan(nw.col("__abs_y__"))).alias(model)
        )
    return result.drop("__abs_y__").to_native()
