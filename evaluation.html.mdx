...
`weights` | <code>str, pandas or polars DataFrame</code> | Weights to use when `agg_fn='weighted_mean'`. If 'auto', weights are computed as the sum of the target in the forecast/evaluation window, not from `train_df`. If a dataframe, it must contain `id_col` and `weight`; if `cutoff_col` is present in `df`, it can also contain `cutoff_col` for cutoff-level weights. If `df` has `cutoff_col` but the weights dataframe does not, the same per-series weight is used for every cutoff. | <code>None</code>
