...
`resampler_kwargs` | <code>[dict](#dict)</code> | Keyword arguments to be passed to plotly-resampler constructor. For further customization ("show_dash") call the method, store the plotting object and add the extra arguments to its `show_dash` method. Defaults to None. | <code>None</code>
