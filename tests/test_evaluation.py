...
def test_basic_losses_with_cutoff(engine):
    ...
    evaluation = evaluate(
        df=cv_df,
        metrics=[mae, mse, rmse, bias, smape],
        models=models,
        cutoff_col="cutoff"
    )
...
@pytest.mark.parametrize("engine", ["pandas", "polars"])
def test_msse_rmsse_baseline_with_cutoff(engine):
    """Test edge case: msse/rmsse baseline calculation with cutoff column.

    This test specifically checks that the baseline (seasonal naive) calculation
    works correctly when cross-validation folds are present.
    """
    cv_df, train_df = generate_cv_series(n_series=5, n_models=2, n_cutoffs=3, engine=engine, seed=42)
    models = ["model0", "model1"]

    # Test msse with cutoff
    msse_result = msse(
        df=cv_df,
        models=models,
        seasonality=7,
    ...
    msse_nw = nw.from_native(msse_result)
    assert msse_nw.shape[0] == expected_rows, (
        f"msse: Expected {expected_rows} rows, got {msse_nw.shape[0]}"
    )
    assert "cutoff" in msse_nw.columns, "msse: cutoff column missing"

    # Test rmsse with cutoff
    rmsse_result = rmsse(
        df=cv_df,
        models=models,
        seasonality=7,
    ...
    rmsse_nw = nw.from_native(rmsse_result)
    assert rmsse_nw.shape[0] == expected_rows, (
        f"rmsse: Expected {expected_rows} rows, got {rmsse_nw.shape[0]}"
    )
    assert "cutoff" in rmsse_nw.columns, "rmsse: cutoff column missing"

    # Verify rmsse = sqrt(msse)
    for model in models:
        np.testing.assert_array_almost_equal(
            rmsse_nw[model].to_numpy(),
            np.sqrt(msse_nw[model].to_numpy()),
            decimal=10,
            err_msg=f"rmsse should equal sqrt(msse) for {model}"
        )

    # Verify all results are finite and positive
    for model in models:
        msse_vals = msse_nw[model].to_numpy()
        rmsse_vals = rmsse_nw[model].to_numpy()
        assert np.all(np.isfinite(msse_vals)), f"msse: non-finite values for {model}"
        assert np.all(np.isfinite(rmsse_vals)), f"rmsse: non-finite values for {model}"
        assert np.all(msse_vals >= 0), f"msse: negative values for {model}"
        assert np.all(rmsse_vals >= 0), f"rmsse: negative values for {model}"
