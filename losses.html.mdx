...
<a
href="https://github.com/Nixtla/utilsforecast/blob/main/utilsforecast/losses.py#L423"
target="_blank" style={{ float: "right", fontSize: "smaller" }}>source</a>
...
```python
for df in [series, series_pl]:
    if isinstance(df, pd.DataFrame):
        df_test = df.assign(unique_id=lambda df: df["unique_id"].astype(str))
    else:
        df_test = df.with_columns(pl.col("unique_id").cast(pl.Utf8))
    mql_df = mqloss(
        df_test, 
        models, 
        [0.1, 0.3],
    ...
    if isinstance(mql_df, pd.DataFrame):
        ...
        null_vals = df.select(pl.all().is_null().sum()).sum_horizontal()
...
<a
href="https://github.com/Nixtla/utilsforecast/blob/main/utilsforecast/losses.py#L541"
target="_blank" style={{ float: "right", fontSize: "smaller" }}>source</a>
...
<a
href="https://github.com/Nixtla/utilsforecast/blob/main/utilsforecast/losses.py#L591"
target="_blank" style={{ float: "right", fontSize: "smaller" }}>source</a>
